...
- 统一 QA Pipeline 服务 (`services/qa_pipeline.py`)。
- Neo4j 数据存储与检索 (`storage/neo4j_storage.py`, `storage/search_service.py`)。
- 多步 (Multi-hop) AI 问答推理 (`api/ai_qa_routes.py`)。
- 报告生成 (`api/report.py`)。
...
### QA Pipeline (`services/qa_pipeline.py`)
统一服务，整合检索、分块、重排功能，支持情景化查询。
...
- `backend/app/services/qa_pipeline.py`: 统一 QA Pipeline 服务。
